...
int main() {
    ...
    cout << "Выберите действие (1 - шифрование или 2 - расшифровка): ";
    cin >> mode;
    if (mode != "1" && mode != "2") {
    ...
    if (mode == "1") {
        cout << "Введите название файла, текст из которого будет задействован для шифрования (либо введите d (default) для использования файла, подготовленного по умолчанию): ";
        cin >> input_file_string;
        if(input_file_string == "d")
            input_file_string = "enctest.txt";
        ...
        cout << "Введите название файла, в который будет записан зашифрованный текст (либо введите d (default) для использования файла, подготовленного по умолчанию): ";
        cin >> output_file_string;
        if(output_file_string == "d")
            output_file_string = "encrypt.txt";
        ...
        cout << "Введите название файла, текст из которого будет задействован для расшифровки (либо введите d (default) для использования файла, подготовленного по умолчанию): ";
        cin >> input_file_string;
        if(input_file_string == "d")
            input_file_string = "encrypt.txt";
        ifstream input_check(input_file_string);
        if(input_check.is_open() == 0) {
        ...
        cout << "Введите название файла, в который будет записан расшифрованный текст (либо введите d (default) для использования файла, подготовленного по умолчанию): ";
        cin >> output_file_string;
        if(output_file_string == "d")
            output_file_string = "decrypt.txt";
        ifstream output_check(output_file_string);
        if(output_check.is_open() == 0) {
